 ...
 const allPositionsEver = data.positions;
  ...
  const scorecard = useMemo(() => {
    ...
    return Object.values(grouped).map(g => {
      ...
      const pendingPremium = openCalls.reduce((s, c) => s + (c.premium || 0) * (c.contracts || 0) * 100, 0);
                            ...
                            <span className={`font-semibold ${premiumColor}`}>
                              ...
                              {s.pendingPremium > 0 && (
                                <span className="text-xs text-gray-400 font-normal ml-2">
                                  + {formatCurrency(s.pendingPremium)} pending
                                </span>
                              )}
                            </span>
                          </div>
                ...
                <div className="overflow-x-auto">
                  {activePositions.length === 0 ? (
                    ...
                    <table className="w-full text-sm">
                      ...
                      <tbody>
                        {activePositions.map((p) => {
                          const pCalls = weightedPremium(p, activePositions, data.calls);
                          const pOpen = pCalls.filter((c) => c.status === "open").length;
